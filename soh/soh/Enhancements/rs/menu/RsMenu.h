...
//
// One stone per page, the ring's page red, 2 above the scroll's rolls and centred on it, at a static height
// that moves only with the scroll's fixed drop. It ramps with the arrival and fades out on the way down a
// level exactly as START does, and is hidden at level 1.
...
struct RsMenuStepperState {
    ...
    int32_t at;
    int32_t stones;
    float stone; // a stone's side: 14, less once the row would be wider than the parchment
    float x0, y0, x1, y1;

#include "CustomArchCutBench100Scene.h"
#include "global.h"
#include "z64scene.h"
#include "macros.h"
...
extern "C" {
    extern CollisionHeader arch_cut_bench_100_scene_collisionHeader;
    extern RoomShapeNormal arch_cut_bench_100_room_0_shapeHeader;
...
static EnvLightSettings sArchCutBench100LightSettings[4] = {
...
static EntranceEntry sArchCutBench100Entrances[] = {
    { 0, 0 },
};
...
static ActorEntry sArchCutBench100PlayerSpawn = {
    ACTOR_PLAYER, { 700, 0, -1240 }, { 0, -32768, 0 }, 0xDFF
};

static RomFile sArchCutBench100RoomList[] = {
    { (uintptr_t)&arch_cut_bench_100_room_0_shapeHeader,
      (uintptr_t)&arch_cut_bench_100_room_0_shapeHeader + 256,
      nullptr },
};

extern "C" int CustomArchCutBench100Scene_IsCustomScene(s32 sceneId) {
    return sceneId == SCENE_ARCH_CUT_BENCH_100;
}
...
static void InitScene(PlayState* play, s32 spawn) {
    ...
    BgCheck_Allocate(&play->colCtx, play, &arch_cut_bench_100_scene_collisionHeader);

    play->numRooms = 1;
    play->roomList = sArchCutBench100RoomList;

    play->setupEntranceList = sArchCutBench100Entrances;
    play->linkActorEntry    = &sArchCutBench100PlayerSpawn;
    ...
    play->envCtx.lightSettingsList = sArchCutBench100LightSettings;

    Play_InitEnvironment(play, play->skyboxId);
    GameInteractor_ExecuteAfterSceneCommands(play->sceneNum);
}

extern "C" void CustomArchCutBench100Scene_InitRoom(PlayState* play, RoomContext* roomCtx) {
    roomCtx->curRoom.echo       = 0;
    roomCtx->curRoom.meshHeader = (MeshHeader*)&arch_cut_bench_100_room_0_shapeHeader;
...
extern "C" int CustomArchCutBench100Scene_TrySpawn(PlayState* play, s32 sceneId, s32 spawn) {
    if (sceneId != SCENE_ARCH_CUT_BENCH_100) {
        return 0;
    }
    ...
    SPDLOG_INFO("CustomArchCutBench100Scene: spawned scene {} spawn {}", sceneId, spawn);
    return 1;
}
